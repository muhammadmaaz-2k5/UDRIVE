...
(async () => {
  try {
    ...
    const loginRes = await makeRequest('/api/users/login', 'POST', {}, loginData);
    ...
    const profileRes = await makeRequest('/api/users/profile', 'GET', {
      'Authorization': `Bearer ${token}`
    });
    console.log('Profile Status:', profileRes.statusCode);
    console.log('Profile Body:', JSON.stringify(profileRes.body));
    if (profileRes.statusCode !== 200 || profileRes.body.email !== 'john.doe@example.com') {
      throw new Error('Profile access failed');
    }

    console.log('\n3. Logging out user (blacklisting token)...');
    const logoutRes = await makeRequest('/api/users/logout', 'GET', {
      'Authorization': `Bearer ${token}`
    });
    ...
    const profileAfterLogoutRes = await makeRequest('/api/users/profile', 'GET', {
      'Authorization': `Bearer ${token}`
    });
